...
function bhv_arena_spring_loop(obj)
    ...
    local player = m and m.marioObj --gMarioStates[0].marioObj
    local dist = player and (math.sqrt((obj.oPosX - player.oPosX) ^ 2 + (obj.oPosY - player.oPosY) ^ 2 + (obj.oPosZ - player.oPosZ) ^ 2)) or 10000
    if dist < 160 then
