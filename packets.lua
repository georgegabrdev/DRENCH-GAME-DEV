...
function send_packet_to_server(p)
	if p.packetType ~= nil and not network_is_server() then -- make sure the packet is valid and the server is not sending the packet
		...
		log_to_console("Drench DX: Sent packet to the server with packet type " .. p.packetType)
...
function send_packet(globalIndex, p)
	...
	elseif network_local_index_from_global(globalIndex) == 0 then
		log_to_console("Drench DX: Tried to send packet to the same player", CONSOLE_MESSAGE_ERROR)
	else
		log_to_console("Drench DX: Tried to send a invalid packet", CONSOLE_MESSAGE_ERROR)
	end
end
...
local function handle_warning_packet(p)
	if network_is_server() then
		create_warning_local(p.data)
		local globalWarning = create_packet(PACKET_TYPE_WARNING_MESSAGE, p.data)
		network_send(true, globalWarning)
	else
		create_warning_local(p.data)
	end
end
...
local function on_packet_receive(p)
	if p.packetType == PACKET_TYPE_WARNING_MESSAGE then
		handle_warning_packet(p)
	end
end
